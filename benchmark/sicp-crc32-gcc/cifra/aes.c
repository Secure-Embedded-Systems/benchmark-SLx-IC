...
#if defined(CF_USE_HW_AES) || defined(CF_USE_DMA_HW_AES)
#include <msp430.h>
#include "driverlib.h"
#endif


#ifdef CF_USE_DMA_HW_AES
//#define NUMBLOCKS 1
void initAESDMA(const uint8_t * cipherKey)
{
    int i;
    // Reset AES
     AESACTL0=AESSWRST;
...
void AES256encDMA( uint8_t * data, uint8_t * encryptedData)
{
    // DMA Triggers
    ...
    __data20_write_long((unsigned long)&DMA0DA, (unsigned long)encryptedData);
    // Channel 0 Size
    DMA0SZ = 8; //NUMBLOCKS*8;
    // Enable Channel 0
    DMA0CTL &= ~DMAIFG;
    DMA0CTL |= DMAIE;
    DMA0CTL |= DMAEN;
...
//void AES256decDMA( uint8_t * data, uint8_t * encryptedData)
//{
////    // Reset AES
////     AESACTL0=AESSWRST;
////     // select key length 128 bit
...
#elif defined(CF_USE_DMA_HW_AES)
	initAESDMA(key);
#else
